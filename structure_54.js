...
const init = async () => {
    ...
    d3.json(
        ...
        dataFlattened.pop();
        resultData = dataFlattened;//[...dataFlattened, ...companyData];
        ...
        chart = new d3.OrgChart()
            ...
            .nodeContent(function (d, i, arr, state) {
                ...
                return `
                <div onclick="zoomElement(${
                    d.data.id
                })" onmouseover="showPopup(${d.data.id})" onmouseleave="leavePopup()" style="cursor: pointer; padding-top:30px;background-color:#00AFC1;margin-left:1px;height:${d.height}px;border-radius:2px;overflow:visible;border-radius:40px;display:flex;justify-content:center;overflow:hidden;">
                <div style="height:${
                    d.height - 62
  ...
  `;
            ...
            .buttonContent(({ node, state }) => {
                return `<div id=${
                    node.data.id
                } class="expand-button" style="color:#2CAAE5;border-radius:5px;padding:3px;font-size:10px;margin:auto auto;background-color:#040910;border: 1px solid #2CAAE5"> <span style="font-size:9px">${
                    node.children
            ...
            .onNodeClick(function (nodeId) {
                ...
                function getNode(id) {
                    var node = chart
                        ...
                        .allNodes.filter(
                            ({ data }) =>
                                chart.getChartState().nodeId(data) == id
                        )[0];
                    return node;
                }
                var node = getNode(nodeId);
        ...
        var showOnMap = function (id) {
            window.location.href = window.mapUI + id;
        };
        var editElement = function (id) {
            window.location.href = window.editUI + id;
        };
        var showOnTree = function (id) {
            window.location.href = window.treeUI + id;
        };
        var menu = [
            {
                title: 'Show on Tree',
        ...
        chart.createContextMenu = function () {
            d3.selectAll('.node').on('contextmenu', event =>
                d3.contextMenu(event, menu)
            );
            return this;
        };
        if (window.searchParam) {
            chart
                .setCentered(window.searchParam)
                .setUpToTheRootHighlighted(window.searchParam)
                .render();
        }
        chart
            .compact(!!(compact++ % 2))
            ...
            .fit();
        document
            .getElementsByClassName('chart-container')[0]
...
async function getCompanies() {
    let url = 'http://192.168.14.203/otcs/llisapi.dll?func=ll&objId=121803356&objAction=RunReport';
...
async function getPopup1(cid, year) {
    try {
        let url = 'http://192.168.14.203/otcs/llisapi.dll?func=ll&objId=121961178&objAction=RunReport';
        url += '&cid=' + cid + '&year=' + year;;
        let result = await fetch(url);
        let data = await result.json();
        return data;
    } catch (e) {
...
async function getPopup2(cid) {
    try {
        let url = 'http://192.168.14.203/otcs/llisapi.dll?func=ll&objId=121961510&objAction=RunReport';
        url += '&cid=' + cid;
        let result = await fetch(url);
        let data = await result.json();
        data.pop();
...
async function getPopup3(cid) {
    try {
        let url = 'http://192.168.14.203/otcs/llisapi.dll?func=ll&objId=121961948&objAction=RunReport';
        url += '&cid=' + cid;
        let result = await fetch(url);
        let data = await result.json();
        data.pop();
...
async function getPopup4(cid) {
    try {
        let url = 'http://192.168.14.203/otcs/llisapi.dll?func=ll&objId=121961951&objAction=RunReport';
        url += '&cid=' + cid;
        let result = await fetch(url);
        let data = await result.json();
        data.pop();
...
async function getPopup5(cid) {
    try {
        let url = 'http://192.168.14.203/otcs/llisapi.dll?func=ll&objId=121961619&objAction=RunReport';
        url += '&cid=' + cid;
        let result = await fetch(url);
        let data = await result.json();
        data.pop();
...
async function getPopup6(cid) {
    try {
        let url = 'http://192.168.14.203/otcs/llisapi.dll?func=ll&objId=121961623&objAction=RunReport';
        url += '&cid=' + cid;
        let result = await fetch(url);
        let data = await result.json();
        return data;
...
let companyStructureImages = [];
    async function getCompanyStructureImages() {
        try{
            let url = 'http://192.168.14.203/otcs/llisapi.dll?func=ll&objId=121803359&objAction=RunReport';
            let result = await fetch(url);
            let data = await result.json();
...
async function showPopup(key) {
    ...
    if (!showPopupTimeout) {
        showPopupTimeout = setTimeout(async () => {
            ...
            const popup1 = await getPopup1(key, new Date().getFullYear());
            ...
            const popup6 = await getPopup6(key);
            ...
            let image = companyStructureImages.find(x=>x.COMPANYID == key);
            ...
            function createAddInfoBox() {
                ...
                if (
                    (data.countemp && data.countemp !== '?') ||
                    (data.countemp2 && data.countemp2 !== '?')
                ) {
                    var numberOfEmployeesTitle = `<div class="row mt-2"><div class="col-6"><b>İşçi sayı:</b></div><div class="col-6">${
                        data.countemp && data.countemp !== '?'
                            ? data.countemp
                            : ''
                    } (${
